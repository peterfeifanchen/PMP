...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        ********** IMPORTANT ***********
        ...
        # LSTM Cell with bias
        self.decoder = nn.LSTMCell(embed_size + hidden_size, hidden_size * 2)

        # Linear Layer with no bias), called W_{h} in the PDF.
        self.h_projection = nn.Linear(hidden_size * 2, hidden_size * 2, bias=False)
        # Linear Layer with no bias), called W_{c} in the PDF.
        self.c_projection = nn.Linear(hidden_size * 2, hidden_size * 2, bias=False)
        # Linear Layer with no bias), called W_{attProj} in the PDF.
        self.att_projection_2 = nn.Linear(hidden_size * 2, hidden_size * 2, bias=False)
        self.att_projection_1 = nn.Linear(hidden_size * 2, hidden_size * 2, bias=False)
        self.att_projection_v = nn.Linear(hidden_size * 2, 1, bias=False)
        # Linear Layer with no bias), called W_{u} in the PDF.
        self.combined_output_projection = nn.Linear(hidden_size * 2 + hidden_size * 2, hidden_size, bias=False)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        # Compute log probability of generating true target word
    ...
    def encode(self, source_padded: torch.Tensor,
        ...
        X = self.source_embeddings(source_padded)  #(src_len, batch, embed_dim)      
        ...
        # LSTM outputs only hidden states with enc_hidden and enc_cells already the last hidden state of each direction
        # concatenated together
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        #(seq_len, batch, num_directions * hidden_size)
        enc_hiddens, _ = torch.nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1,0,2)
        #(num_layer * num_direction, batch, hidden_size)
        last_hidden = torch.cat( (last_hidden[0,:,:], last_hidden[1,:,:]), dim=1)
        last_cell = torch.cat( (last_cell[0,:,:], last_cell[1,:,:]), dim=1)
        # concat forward/backward hidden state vertically in the hidden state dimension
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection_1(enc_hiddens)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #     Tensor Squeeze:
        #         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        dec_hidden_proj = self.att_projection_2(dec_hidden)
        sum_dec_enc_proj = torch.tanh( dec_hidden_proj.unsqueeze(1) + enc_hiddens_proj )
        e_t = self.att_projection_v( sum_dec_enc_proj ).squeeze(-1)
        # END YOUR CODE
    ...
    def beam_search(self, src_sent: List[str],
        ...
        src_encodings_att_linear = self.att_projection_1(src_encodings)
