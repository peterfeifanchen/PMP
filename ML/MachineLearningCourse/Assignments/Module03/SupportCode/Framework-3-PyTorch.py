...
# Create the model
model = BlinkNeuralNetwork.BlinkNeuralNetwork(hiddenNodes = [5,5])
...
while not converged and epoch < maxEpoch:
    # Do the forward pass
    ...
    print("epoch %d: training loss {}, validation loss {}".format(epoch, trainLoss.item(), validationLoss.item()))
    if lastValidationLoss is not None and np.abs(validationLoss.item() - lastValidationLoss) < convergence:
        converged = True
        pass
    else:
...
Charting.PlotSeries([trainLosses, validationLosses], ["Train Loss", "Validate Loss"], xValues, useMarkers=False, chartTitle="Pytorch First Modeling Run", xAxisTitle="Epoch", yAxisTitle="Loss", yBotLimit=0.0, outputDirectory=kOutputDirectory, fileName="3-PyTorch-Initial-TrainValidate")
