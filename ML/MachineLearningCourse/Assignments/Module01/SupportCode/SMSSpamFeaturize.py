...
class SMSSpamFeaturize(object):
    ...
    def FindMostFrequentWords(self, x, n):
        ...
        for sentence in x:
            ...
            for word in words:
                c[word] += 1
        return [ wordtoken[0] for wordtoken in c.most_common(n)]
    ...
    def FindTopWordsByMutualInformation(self, x, y, n):
        ...
        for word in c:
            ...
            mi[word] = pWordAnd0 * np.log( pWordAnd0 / (p0*pWord) ) + pWordAnd1 * np.log( pWordAnd1 / (p1*pWord) ) + \
                pNotWordAnd0 * np.log(pNotWordAnd0 / (p0*pNotWord) ) + pNotWordAnd1 * np.log( pNotWordAnd1 / (p1*pNotWord) )
    ...
    def CreateVocabulary(self, xTrainRaw, yTrainRaw, numFrequentWords=0, numMutualInformationWords=0, supplementalVocabularyWords=[]):
        ...
        mostCommonWords = self.FindMostFrequentWords(xTrainRaw, numFrequentWords)
        highestMutualInfoWords = self.FindTopWordsByMutualInformation(xTrainRaw, yTrainRaw, numMutualInformationWords)
        self.vocabulary = mostCommonWords + highestMutualInfoWords
